...
function renderRoomList(rooms: { [key: string]: RoomInfo }) {
  ...
  Object.values(rooms).forEach((room, index) => {
    ...
    row.innerHTML = `
      ...
      <td class="px-4 py-2 participant-count">${memberCount}/5</td>
...
async function updateCurrentRoomInfo(members?: {
  ...
  try {
    ...
    if (members) {
      ...
      if (memberCount > 5) {
        console.warn(`참여 인원이 제한 초과: ${memberCount}명`);
        memberCount = 5; // UI에는 최대 5명으로 표시
      }
    } else {
      ...
      if (room) {
        ...
        console.warn(
          `방 ID [${currentRoomId}]에 해당하는 방 정보를 찾을 수 없습니다.`
        );
        return;
      }
    }
    ...
    console.log(`방 [${currentRoomId}] 참여 인원 업데이트: ${memberCount}명`);
...
joinRoomBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (participantCount >= 5) {
      alert(
        "참여 인원이 5명 이상인 방에는 입장할 수 없습니다. 다른 방에 입장해주세요."
      );
      return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  socket.on(
    ...
    (members: { [key: string]: { user_id: string; nickName: string } }) => {
      ...
      const lastUserId = memberIds[memberIds.length - 1];

      if (currentUserId === lastUserId || memberCount > 6) {
        alert("참여 인원 제한으로 자동 퇴장됩니다.");
        leaveRoom();
        connectedRoomElem.textContent = "";
        return;
      }
...
async function loadRoomListExceptCurrent(currentRoomId: string) {
  try {
    ...
    // 현재 방의 참여인원 정보 백업
    const currentParticipantCell = document.querySelector(
      `tr[data-room-id="${currentRoomId}"] .participant-count`
    );
    const currentParticipantText = currentParticipantCell
      ? currentParticipantCell.textContent
      : null;
    ...
    if (currentParticipantText) {
      const updatedParticipantCell = document.querySelector(
        `tr[data-room-id="${currentRoomId}"] .participant-count`
      );
      if (updatedParticipantCell) {
        updatedParticipantCell.textContent = currentParticipantText;
        console.log(
          `현재 방 [${currentRoomId}]의 참여인원 정보 복원: ${currentParticipantText}`
        );
      }
    }
  } catch (error) {
