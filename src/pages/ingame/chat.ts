...
async function findRoomByName(searchName: string): Promise<RoomInfo | null> {
  try {
    ...
    const foundRoom = Object.values(rooms).find(
      (room) => room.roomName.toLowerCase() === searchName.toLowerCase()
    );
...
async function loadRoomList() {
  try {
    ...
    Object.values(rooms).forEach((room) => {
      ...
      const serverCount = room.memberList
        ? Object.keys(room.memberList).length
        : 0;
    ...
    if (currentRoom) {
      ...
      if (
        ...
        deletedRoomIds.includes(currentRoom.roomId)
      ) {
...
async function updateCurrentRoomInfo(members?: {
  [key: string]: { user_id: string; nickName: string };
}) {
  console.log("updateCurrentRoomInfo 호출됨", members);
  const currentRoom = loadCurrentRoom();
  if (!currentRoom) {
    return;
  }
  ...
  try {
    ...
    if (members) {
      ...
      console.log(
        "멤버 닉네임 목록:",
        Object.values(members).map((m) => m.nickName)
      );
    } else {
      const rooms = await getRooms();
      ...
      if (roomInfo) {
        const serverCount = roomInfo.memberList
          ? Object.keys(roomInfo.memberList).length
          : 0;
        const cachedCount = roomParticipantCache[currentRoomId] || 0;
        memberCount = Math.max(serverCount, cachedCount);
        roomParticipantCache[currentRoomId] = memberCount;
        saveCacheToStorage();
        // 서버에서 받은 멤버 닉네임 출력 예빈
        if (roomInfo.memberList) {
          console.log(
            "서버 멤버 닉네임 목록:",
            Object.values(roomInfo.memberList).map((m) => m.nickName)
          );
        }
      } else {
        return;
...
joinRoomBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (newRoomName) {
      ...
      if (
        ...
        deletedRoomIds = deletedRoomIds.filter((id) => {
          ...
          return (
            !room || room.roomName.toLowerCase() !== newRoomName.toLowerCase()
          );
        });
        localStorage.setItem(DELETED_ROOMS_KEY, JSON.stringify(deletedRoomIds));
        ...
        if (joinResult && joinResult.ok) {
          ...
          alert(
            `채팅방은 생성되었지만 입장에 실패했습니다: ${joinResult?.message || "알 수 없는 오류"}`
          );
        }
      } else {
    ...
    // 2. 기존 방 입장 모드
      ...
      if (!foundRoom) {
        alert(
          `"${existingRoomName}" 방이 존재하지 않습니다. 채팅방을 생성하려면 '생성할 방 이름'에 입력해주세요.`
        );
        return;
      }
      ...
      if (deletedRoomIds.includes(foundRoom.roomId)) {
        alert(
          `"${existingRoomName}" 방은 이미 삭제되었습니다. 새로운 방을 생성해주세요.`
        );
        return;
      }
      ...
      // 참여자 수 확인
      const serverCount = foundRoom.memberList
        ? Object.keys(foundRoom.memberList).length
        : 0;
      ...
      if (result.ok) {
        ...
        // 추가: 멤버 정보 명시적 요청 또는 빈 메시지 전송
        setTimeout(() => {
          sendMsg(" "); // 빈 메시지로 members 이벤트 트리거
        }, 300);
  ...
  try {
    ...
    if (!currentRoom) {
      ...
      currentRoom = {
        ...
        memberCount: foundRoom.memberList
          ? Object.keys(foundRoom.memberList).length
          : 1,
      };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  socket.on(
    ...
    (members: { [key: string]: { user_id: string; nickName: string } }) => {
      console.log("members 이벤트 발생!", members); // 이벤트 수신 확인용 로그

      const currentRoom = loadCurrentRoom();
      console.log("currentRoom:", currentRoom); // currentRoom 상태 확인용 로그
