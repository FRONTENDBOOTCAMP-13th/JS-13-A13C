...
| 구분           | 기능 설명                                                                 |
|----------------|--------------------------------------------------------------------------|
| 방 관리        | 방 생성 혹은 기존 방 입장 가능                                           |
| 닉네임 설정    | 사용자 닉네임 입력 및 같은 방 유저 리스트 실시간 표시                    |
| 실시간 채팅    | 같은 방의 사용자들과 텍스트 채팅 가능                                     |
| 게임 진행      | 1~8번 카드 중 2장을 제출하고, 1장을 선택하여 사용 (선택 후 변경 불가)     |
| 게임 동기화    | 모든 플레이어의 선택 상태 및 결과가 실시간으로 반영                      |
| 점수 시스템    | 점수 및 라운드 승리 횟수 자동 계산 및 시각화                             |
| 카드 관리      | 사용된 카드는 제거, 미사용 카드는 한 라운드 임시 보관 후 재사용 가능     |
| 결과 처리      | 5라운드 종료 후 승점 → 라운드 승리 횟수 → 마지막 승리자 순으로 우승자 결정 |
